...
void Framework::ui::Render()
{
    g.begin("Main Window", window::size_max);
    {
        ...
        c.Text(window::size_max.x - 120, window::size_max.y - 30, fonts::Inter_Regular, 15, "modded by no one ;)", colors::White);
        s.Alpha = alphaColor;

        if (TAB == 0)
        {
            alphaColor = std::clamp(alphaColor + (1.f * ImGui::GetIO().DeltaTime * 1.f), 0.0f, 1.f);
            c.ShadowText(190, 140, fonts::Sansation_Bold, 100, "lame as ui", colors::White, colors::MainColor);
            ...
            if (c.Button("Login", "Sign in", 190, 260, 250, 30)) { TAB = 1; alphaColor = 0; }
        }
